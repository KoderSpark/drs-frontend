...
export const api = {
  ...
  contacts: {
    create: (payload: { name: string; email: string; phone?: string; message: string }) =>
      request('/api/contacts', { method: 'POST', headers: buildHeaders(true), body: JSON.stringify(payload) }),
  ...
  admin: {
    ...
    login: (payload: { email: string; password: string }) =>
      request('/api/admin/login', { method: 'POST', headers: buildHeaders(true), body: JSON.stringify(payload) }),
  },
  ...
  doctors: {
    ...
    login: (payload: { email: string; password: string }) =>
      request('/api/doctors/login', { method: 'POST', headers: buildHeaders(true), body: JSON.stringify(payload) }),
    list: (status?: string) => {
      const q = status ? `?status=${encodeURIComponent(status)}` : '';
      return request(`/api/doctors${q}`);
    },
    get: (id: string) => request(`/api/doctors/${id}`, { headers: buildHeaders(true) }),
  updateProfile: (id: string, form: FormData) => request(`/api/doctors/${id}/profile`, { method: 'PATCH', headers: buildHeaders(false), body: form }),
  update: (id: string, form: FormData) => request(`/api/doctors/${id}`, { method: 'PATCH', headers: buildHeaders(false), body: form }),
